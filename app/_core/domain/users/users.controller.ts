  ...
  routes(): Router {
    this.router.get('/sections', async (req: Request, res: Response) => {
      ...
      return response.hasError()
        ...
        : this.successResponse(res, response.getData('users'))
    })
    ...
    this.router.get('/:uid', async (req: Request, res: Response) => {
      ...
      return response.hasError()
        ? this.errorResponse(res, response.jsonErrors())
        : this.successResponse(res, response.getData('user'))
    })
    ...
    this.router.post('/', validate(CreateUserSchema), async (req: Request, res: Response) => {
      ...
      return response.hasError()
        ? this.errorResponse(res, response.jsonErrors())
        : this.successResponse(res, response.getData('user'), StatusCodeUtils.CREATED)
    })
    ...
    this.router.put('/', validate(UpdateUserSchema), async (req: Request, res: Response) => {
      ...
      return response.hasError()
        ? this.errorResponse(res, response.jsonErrors())
        : this.successResponse(res, response.getData('user'), StatusCodeUtils.OK)
    })
