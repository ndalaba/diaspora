...
import AssociationRepository, {AssociationMemberRepository} from "../associations/association.repository";
...
export default class UserService {
    ...
    async createUser(dto: CreateUserDto): Promise<Response> {
        return tryCatch(async () => {
            ...
            if (dto.association_uid !== undefined && dto.association_uid !== "") {
                ...
                const associationMember = new AssociationMember({member: user, association: association});
    ...
    async updateUser(dto: UpdateUserDto): Promise<Response> {
        ...
        return tryCatch(async () => {
            ...
            let user = await this.userRepo.getOrFail(dto.uid) as User;
            if (!this.canUpdate(user, dto.user, response))
                return response;
