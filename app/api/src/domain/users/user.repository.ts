...
export class UserRepository extends AbstractRepository<User> {
    ...
    filter(dto: FilterUserDto): Promise<User[]> {
        const query = this.repository.createQueryBuilder("users")
            .innerJoin("association_members", "association_members", "association_members.user_id = users.id")
            .innerJoin("associations", "associations", "association_members.association_id = associations.id")
            .leftJoinAndSelect("users.country", "countries")
        ;
        ...
        if (dto.firstName && dto.firstName.trim().length)
            query.andWhere("users.firstName LIKE :firstName", {firstName: `'%${dto.firstName}%'`});

        if (dto.lastName && dto.lastName.trim().length)
            query.andWhere("users.lastName LIKE :lastName", {lastName: `'%${dto.lastName}%'`});

        if (dto.city && dto.city.trim().length)
            query.andWhere("users.city LIKE :city", {city: `'%${dto.city}%'`});

        if (dto.country_id && +dto.country_id !== 0) {
            query.innerJoinAndSelect("users.country", "countries")
                .andWhere("users.country_id = :country_id", {country_id: dto.country_id});
        }

        query.andWhere("associations.id = :association_id", {association_id: dto.association_id});
