...
export default class AssociationService {
  constructor(private readonly associationRepo: AssociationRepository = new AssociationRepository(),
  ...
  async updateAssociationLogo(dto: UpdateImage): Promise<Response> {
    ...
    }, dto.uid, "update association logo");
  }
  ...
  async updateAssociation(dto: UpdateAssociationDto): Promise<Response> {
    ...
    return tryCatch(async () => {
      ...
      let association = await this.associationRepo.getOrFail(dto.uid) as Association;
      if (!canUpdate(association, dto.user, response))
        return response;
...
export class AssociationMemberService {
  ...
  async setAdmin(dto: AddMemberDto): Promise<Response> {
    ...
    if (memberSections.length > 0)
      return new Response().addError("already", "Member already added");

    const sections = await this.sectionRepo.findPublished();

    const userSections: Partial<UserSection[]> = [];
    sections.forEach(mod => userSections.push(new UserSection({
      read: false,
      write: false,
  ...
  async removeMemberToAssociation(dto: AddMemberDto): Promise<Response> {
    ...
    return tryCatch(async () => {
      ...
      const memberAssociation = await this.associationMemberRepo.findByAssociationMember(association, member);
      if (memberAssociation !== null)
        await this.associationMemberRepo.remove(memberAssociation);
  ...
  async addMember(dto: AddMemberDto): Promise<Response> {
    ...
    return tryCatch(async () => {
      ...
      const associationMember = new AssociationMember({ member: member, association: association });
