...
export class CountryRepository extends AbstractRepository<Country> {
    ...
    findByNameOrCode(name: string, code: string): Promise<Country | null> {
        return this.repository
            ...
            .where("countries.name = :name OR countries.code = :code", {name: name, code: code})
            .getOne();
    }
    ...
    async countAssociations<T>(value: T): Promise<number> {
        ...
        if (value instanceof Country)
            query.where("associations.country_id = :id", {id: value.id});

        else if (typeof value === "number")
            query.where("associations.country_id = :id", {id: value});
    ...
    async countUsers<T>(value: T): Promise<number> {
        ...
        if (value instanceof Country)
            query.where("users.country_id = :id", {id: value.id});

        else if (typeof value === "number")
            query.where("users.country_id = :id", {id: value});
