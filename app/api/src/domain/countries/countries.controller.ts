...
export default class CountriesController extends AbstractController {
    constructor(private readonly countryService: CountryService = new CountryService(), private readonly router = Router()) {
        super();
    }

    routes(): Router {
        this.router.post("/", adminMiddleware, validate(CreateCountrySchema), async (req: Request, res: Response) => {
            const response = await this.countryService.createCountry(req.body);
            return response.hasError() ? this.errorResponse(res, response.jsonErrors()) : this.successResponse(res, response.getData("country"), StatusCodeUtils.CREATED);
        });

        this.router.put("/", adminMiddleware, validate(UpdateCountrySchema), async (req: Request, res: Response) => {
            const response = await this.countryService.updateCountry(req.body);
            return response.hasError() ? this.errorResponse(res, response.jsonErrors()) : this.successResponse(res, response.getData("country"), StatusCodeUtils.OK);
        });
